...
const tripId =
  new URLSearchParams(location.search).get("id") || "luxor";

const databaseId =
  tripId === "luxor" ? 1 : Number(tripId);

const ar =
  document.documentElement.lang === "ar";

fetch(
  `${SUPABASE_URL}/rest/v1/trips?select=*&id=eq.${databaseId}`,
  {
    headers: {
      apikey: SUPABASE_KEY,
...
.then(rows => {
  ...
  const title =
    ar ? trip.title_ar : trip.title_en;

  const price =
    ar ? trip.price_ar : trip.price_en;

  const description =
    ar ? trip.description_ar : trip.description_en;

  const image =
    ar ? trip.image_ar : trip.image_en;
  ...
  // DESCRIPTION
  const descEl =
    document.getElementById("tripDesc");
